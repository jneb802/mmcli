...
func (mp *modpackModel) loadFromDisk(modpackPath string) {
	...
	if err != nil {
		mp.err = fmt.Errorf("cannot read manifest.json: %w", err)
		return
	}
	var manifest modpackManifest
	if err := json.Unmarshal(data, &manifest); err != nil {
		mp.err = fmt.Errorf("invalid manifest.json: %w", err)
		return
	}
	...
	if readmeData, err := os.ReadFile(filepath.Join(modpackPath, "README.md")); err == nil {
		mp.readme = string(readmeData)
		mp.readmeLines = strings.Split(mp.readme, "\n")
	}
...
func (m model) viewModpackMods() string {
	...
	b.WriteString("\n")
	hotkeys := []string{"↑/↓ navigate", "tab next", "` mode", "q quit"}
	renderHotkeyBar(&b, hotkeys, m.width)
	return b.String()
}
